...
const buildClient = (
  overrides: Partial<TextClientResult> = {}
): TextClientResult => ({
  provider: 'openrouter',
...
describe('AiTextService', () => {
  ...
  describe('generatePromptForPicture', () => {
    ...
    it('deve omitir temperature em reasoning model (gpt-5.5)', async () => {
      factory.text.mockResolvedValue(
        buildClient({ modelId: 'openai/gpt-5.5' })
      );
      generateTextMock.mockResolvedValue({ text: 'ok' });
  ...
  describe('Instagram com provider OpenAI compativel', () => {
    it('gera e valida DNA via JSON em texto livre', async () => {
      factory.text.mockResolvedValue(
        buildClient({
        ...
        text: `\`\`\`json
{
  ...
  "description": "Plataforma de conteudo visual.",
  "targetAudience": "Criadores e marcas.",
  "communicationStyle": "Inspirador e direto.",
  "valueProposition": "Ajudar pessoas a criar e compartilhar.",
  "differentiators": ["Ecossistema visual"],
  "contentPillars": ["Criacao", "Educacao", "Comunidade"],
  "offers": [],
  "visualDirection": {
    "mood": "Vibrante",
    "palette": ["#000000", "#FFFFFF"],
    "imagery": "Criadores em acao",
    "typography": "Sans serif"
  },
  "strategySummary": "Inspirar criacao.",
  "confidence": "high"
}
\`\`\``,
      });

      const result = await service.generateInstagramBrandDna(
        'org-1',
        'instagram',
        'evidencias'
      );
      ...
      expect(result.contentPillars).toHaveLength(3);
      expect(generateObjectMock).not.toHaveBeenCalled();
      expect(generateTextMock).toHaveBeenCalledWith(
        expect.objectContaining({
          system: expect.stringContaining('SOMENTE com um objeto JSON valido'),
        })
      );
    });
  });
  ...
  describe('normalizacao de erro do provedor (evita modal de billing/logout)', () => {
    ...
    it('deve traduzir 402 mesmo sem palavras-chave de credito (nunca vaza 402 cru)', async () => {
      factory.text.mockResolvedValue(buildClient());
      generateTextMock.mockRejectedValue(
        providerError('Payment Required', 402)
      );
