...
describe('AiImageService', () => {
  ...
  describe('generate (OpenRouter)', () => {
    it('deve enviar modalities + image_config com aspect_ratio default 1:1', async () => {
      ...
      const callBody = JSON.parse(
        ((fetchSpy.mock.calls[0] as any)[1] as any).body
      );
  ...
  describe('generate (OpenAI direto)', () => {
    ...
    it('deve usar size 1024x1536 (vertical) quando aspectRatio = 9:16', async () => {
      ...
      const fetchSpy = jest.fn(
        async () =>
          new Response(JSON.stringify({ data: [{ b64_json: 'V' }] }), {
            status: 200,
          })
      );
      globalThis.fetch = fetchSpy as any;
    ...
    it('deve usar default model gpt-image-2 e size 1024x1024 quando nada configurado', async () => {
      ...
      const fetchSpy = jest.fn(
        async () =>
          new Response(JSON.stringify({ data: [{ b64_json: 'B' }] }), {
            status: 200,
          })
      );
      globalThis.fetch = fetchSpy as any;
    ...
    it('deve encerrar timeout do provedor com status 504', async () => {
      resolver.resolve.mockResolvedValue(
        credentialFor({
          provider: 'openai',
          model: 'gpt-image-2',
      ...
      await expect(service.generate('org-1', 'x')).rejects.toMatchObject({
        status: 504,
      });
    });
  });
