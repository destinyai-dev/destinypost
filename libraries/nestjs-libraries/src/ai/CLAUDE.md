...
This rule also covers **AI provider runtime errors** (e.g. OpenRouter/OpenAI returning HTTP 402 "out of credits", 401 bad key, 429 rate limit). `AiTextService.callWithFallback` calls `normalizeProviderError(error)` after both primary and fallback attempts fail. That method:

- Passes `HttpException` through unchanged (already controlled — e.g. 412 "credential not configured" from the resolver).
- Converts any error carrying a numeric `.statusCode` / `.status` (AI SDK `APICallError`) into `new HttpException(friendlyMessage, 412)` with a sanitized pt-BR message.
- Re-throws as-is when there is no numeric status (parse/network errors → NestJS 500).

**Why this matters:** NestJS `BaseExceptionFilter.isHttpError` surfaces any object with `{statusCode, message}` as an HTTP response using that status. Without normalization, a provider 402 leaks as the app's HTTP 402 (opens the billing modal), and a provider 401 logs the user out. Always ensure provider errors reach the client as 412.
...
12. **Symptom:** image generation fails and no backend log appears → **Cause:** historically `AiImageService` only logged provider 4xx/5xx (`_logger.warn`); validation throws, "did not return image" empty responses, and `fetch` network errors were silent. **Fix:** every code path in `AiImageService.generate` (and its delegates `generateOpenAi`/`generateOpenAiEdit`/`generateOpenRouter`) now logs: (a) a `LOG` line at entry (`mode/provider/model/aspect/promptLen/hasRef/profile`), (b) `WARN` on validation rejects (e.g., missing `referenceImageUrl` for I2I), (c) `WARN` when provider responds 200 but with empty `b64_json`/`image_url` (includes `finish_reason` for OpenRouter — helps diagnose "selected model doesn't support image generation"), (d) `ERROR` on `fetch` network failures. `MediaService.generateImage` also wraps `_aiImageService.generate` in `try/catch` and emits `ERROR` with mode/aspect/hasRef/status before re-throwing. When debugging silent failures, grep the backend log for `[AiImageService] generate start` — every request must hit this line; absence means the call never reached the service (probably 412/credit/auth in the controller chain).
13. **Symptom:** provider 402/401/429 leaks as the app's HTTP status (billing modal opens, or user gets logged out) after a text-generation call → **Cause:** NestJS `BaseExceptionFilter.isHttpError` detects any object with `{statusCode, message}` and re-uses that status as the HTTP response. AI SDK `APICallError` carries `.statusCode` for all provider HTTP errors. **Fix:** all text-generation paths go through `AiTextService.callWithFallback`, which calls the private `normalizeProviderError(error)` on failure. That method converts provider errors with a numeric status into `HttpException(friendlyMessage, 412)`. If you add a new text-generation method that bypasses `callWithFallback`, you must wrap provider calls with the same normalization — or call `callWithFallback` directly. `AiImageService` uses direct `fetch` (not AI SDK) and already handles provider 4xx explicitly; `AiVideoService` translates kie.ai error codes in `translateKieaiError`.
