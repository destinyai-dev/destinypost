#!/bin/bash
# Hook: bloqueia gh pr <mutating> sem --repo destinyai-dev/destinypost
...
if echo "$cmd" | grep -qE '\bgh[[:space:]]+pr[[:space:]]+(create|edit|merge|close|ready|review|comment|reopen)\b'; then
  if ! echo "$cmd" | grep -qE -- '--repo[[:space:]]+destinyai-dev/destinypost\b'; then
    echo 'BLOQUEADO: gh pr <create|edit|merge|close|ready|review|comment|reopen> SEM --repo destinyai-dev/destinypost.' >&2
    echo 'Este repo e fork de gitroomhq/postiz-app; sem --repo, o comando vai contra o upstream publico.' >&2
    echo 'Reescreva: gh pr <subcomando> --repo destinyai-dev/destinypost --base main --head <branch> ...' >&2
    exit 2
  fi
fi
