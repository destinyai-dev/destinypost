...
export class CredentialsController {
  ...
  private profileIdForProvider(provider: string, profile: Profile | null) {
    // Canva identifies the OAuth application for the whole installation.
    // Keeping it at organization level avoids a different client secret for
    // each operational profile while the connected Canva account remains
    // organization-wide.
  ...
  async list(
    ...
    const profileCredentials = await this._credentialService.listByOrg(
      org.id,
      profile?.id
    );
    const organizationCredentials = await this._credentialService.listByOrg(
      org.id,
      undefined
    );
    const merged = new Map(
      [...organizationCredentials, ...profileCredentials].map((item) => [
        item.provider,
        item,
      ])
    );
  ...
  @Get('/polotno-key')
  async getPolotnoKey(@GetOrgFromRequest() org: Organization) {
    const stored = await this._credentialService.getRaw(
      org.id,
      'polotno'
    );
    return {
      key: stored?.apiKey || process.env.NEXT_PUBLIC_POLOTNO || '',
    };
  }
  ...
  async getByProvider(
    ...
    const result = await this._credentialService.getRedacted(
      org.id,
      provider,
      this.profileIdForProvider(provider, profile)
    );
    if (!result) {
  ...
  async create(
    ...
    const profileId = this.profileIdForProvider(provider, profile);
    await this._credentialService.save(org.id, provider, body, profileId);
    const result = await this._credentialService.getRedacted(
      org.id,
      provider,
      profileId
