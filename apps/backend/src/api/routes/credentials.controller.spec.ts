...
describe('CredentialsController', () => {
  ...
  describe('organization-level credentials', () => {
    it.each(['canva', 'pexels', 'polotno'])(
      'salva %s no nivel da instalacao, sem vincular ao perfil',
      async (provider) => {
        credentialService.getRedacted.mockResolvedValue({
          data: { apiKey: '__REDACTED__' },
          updatedAt: new Date('2026-07-31T00:00:00.000Z'),
        });

        await controller.create(org, profile, provider, {
          apiKey: 'secret',
        });

        expect(credentialService.save).toHaveBeenCalledWith(
          org.id,
          provider,
          { apiKey: 'secret' },
          undefined
