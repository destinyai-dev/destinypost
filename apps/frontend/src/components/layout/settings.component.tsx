...
import { SelfHostedSetup } from '@gitroom/frontend/components/settings/self-hosted-setup.component';
export const SettingsPopup: FC<{
  getRef?: Ref<any>;
}> = (props) => {
  const { isGeneral, selfHosted } = useVariables();
  const { getRef } = props;
  ...
  const [tab, setTab] = useState(
    () => url.get('tab') || (selfHosted ? 'setup' : 'global_settings')
  );

  const t = useT();
  const list = useMemo(() => {
    const arr = [];
    if (selfHosted && isOrgAdmin) {
      arr.push({ tab: 'setup', label: t('setup_tab', 'Configuração inicial') });
    }
    if (isOrgAdmin) {
        ...
        <FormProvider {...form}>
          <form onSubmit={form.handleSubmit(submit)}>
            ...
            >
              ...
              {tab === 'setup' && selfHosted && isOrgAdmin && (
                <SelfHostedSetup onNavigate={setTab} />
              )}
              {tab === 'teams' &&
                (isOrgAdmin
