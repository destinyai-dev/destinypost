...
import { CanvaDesignsComponent } from '@gitroom/frontend/components/third-parties/canva.designs.component';
...
export const ThirdPartyComponent = () => {
  ...
  const canvaIntegration = data?.find(
    (item: any) => String(item.identifier || '').toLowerCase() === 'canva'
  );
  ...
  return (
    <>
                ...
                {!isLoading && !data?.length ? (
                  ...
                  data?.map((p: any) => (
                    ...
                    >
                      ...
                      >
                        <ImageWithFallback
                          fallbackSrc={`/icons/third-party/${p.identifier}.${
      ...
      <div className="bg-newBgColorInner flex-1 flex-col flex p-[20px] gap-[12px]">
        {canvaIntegration && (
          <CanvaDesignsComponent
            accountName={canvaIntegration.name}
            onDisconnected={() => mutate()}
          />
        )}
        <ThirdPartyListComponent
          reload={mutate}
          connectedIdentifiers={data?.map((item: any) => item.identifier) || []}
        />
      </div>
    </>
  );
