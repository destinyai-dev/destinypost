...
export const ApiModal: FC<{
  ...
  const submit = useCallback(
    async (data: FieldValues) => {
      ...
      if (add.ok) {
        toaster.show(
          t('integration_added', 'Integration added successfully'),
          'success'
        );
        if (closePopup) {
          closePopup();
...
export const ThirdPartyListComponent: FC<{
  ...
  const toaster = useToaster();
  const t = useT();
  const { reload, connectedIdentifiers = [] } = props;
  const normalizedConnectedIdentifiers = connectedIdentifiers.map(
  ...
  const addApiKey = useCallback(
    (title: string, identifier: string) => () => {
      if (identifier === 'canva') {
        ...
        fetch('/third-party/canva/oauth/start')
          .then(async (response) => {
            const data = await response.json();

            if (!response.ok || !data.authorizationUrl) {
              throw new Error(
                data.message || 'Não foi possível iniciar a conexão com o Canva'
              );
            }
  ...
  return (
    <div className="grid grid-cols-4 gap-[10px] justify-items-center justify-center">
      {data
        ?.filter(
          (p: any) =>
            !normalizedConnectedIdentifiers.includes(
          ...
          >
            <div>
              <img
                ...
                src={`/icons/third-party/${p.identifier}.${
                  p.identifier === 'canva' ? 'svg' : 'png'
                }`}
              />
            </div>
            ...
            <div className="whitespace-pre-wrap text-left">
              {String(t(`thirdparty_desc_${p.identifier}`, p.description))}
            </div>
            <div className="w-full flex">
              <Button className="w-full" loading={connecting === p.identifier}>
                {p.identifier === 'canva' ? 'Conectar Canva' : t('add', 'Add')}
              </Button>
            </div>
          </div>
        ))}
